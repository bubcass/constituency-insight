...
export function mountMastheadActions({title, text} = {}) {
  ...
  menu.hidden = true;
  button.classList.add("oireachtas-masthead__menu-action");
  themeButton.classList.add("oireachtas-masthead__menu-action");
  button.insertAdjacentHTML("beforeend", "<span>Share</span>");
  const updateThemeLabel = () => { const label = document.documentElement.dataset.theme === "dark" ? "Light mode" : "Dark mode"; themeButton.querySelector("span")?.remove(); themeButton.insertAdjacentHTML("beforeend", `<span>${label}</span>`); };
  updateThemeLabel();
  themeButton.addEventListener("click", updateThemeLabel);
  menu.append(button, themeButton);
  more.addEventListener("click", () => { menu.hidden = !menu.hidden; more.setAttribute("aria-expanded", String(!menu.hidden)); });
  actions.append(more, menu, status);
