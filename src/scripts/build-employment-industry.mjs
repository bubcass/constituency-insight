...
const outputPath = path.join(dataDir, "employment-industry-2022.csv");
...
const dimensions = Object.fromEntries(dataset.id.map((id, index) => [id, {id, index, size: dataset.size[index], ...dataset.dimension[id]}]));
const industryDimension = dimensions.C03763V04512;
const sexDimension = dimensions.C03738V04487;
const geographyDimension = dimensions.C04167V04938;

if (!industryDimension || !sexDimension || !geographyDimension) {
  throw new Error("The CSO dataset dimensions have changed");
}

const industries = orderedCategories(industryDimension);
const sexes = orderedCategories(sexDimension);
const geographies = orderedCategories(geographyDimension);
const industryByLabel = new Map(industries.map((d) => [d.label, d]));
const sexByLabel = new Map(sexes.map((d) => [d.label, d]));
const totalIndustry = requiredCategory(industryByLabel, "Total");
...
const sectorIndustries = industries.filter((d) => d.code !== totalIndustry.code);
...
const outputRows = electoralDivisions.map((geography) => {
  ...
  for (const industry of industries) {
    for (const sex of sexes) {
      row[fieldName(industry.label, sex.label)] = valueAt(dataset, {
        [industryDimension.id]: industry.position,
  ...
  for (const sex of sexes) {
    const sectorSum = sectorIndustries.reduce((sum, industry) => sum + row[fieldName(industry.label, sex.label)], 0);
    const expected = row[fieldName(totalIndustry.label, sex.label)];
    if (sectorSum !== expected) {
      throw new Error(`${geography.label}: ${sex.label} industries sum to ${sectorSum}, expected ${expected}`);
    }
  }

  for (const industry of industries) {
    const sexSum = row[fieldName(industry.label, male.label)] + row[fieldName(industry.label, female.label)];
    const expected = row[fieldName(industry.label, bothSexes.label)];
    if (sexSum !== expected) {
      throw new Error(`${geography.label}: ${industry.label} male and female values sum to ${sexSum}, expected ${expected}`);
    }
  }
...
const edTotal = outputRows.reduce((sum, row) => sum + row[fieldName(totalIndustry.label, bothSexes.label)], 0);
const nationalTotal = valueAt(dataset, {
  [industryDimension.id]: totalIndustry.position,
  [sexDimension.id]: bothSexes.position,
  [geographyDimension.id]: ireland.position
});

if (edTotal !== nationalTotal) {
  throw new Error(`ED totals sum to ${edTotal}, but the CSO Ireland total is ${nationalTotal}`);
}
...
console.log(`Validated persons at work against the CSO Ireland total: ${nationalTotal.toLocaleString("en-IE")}`);
...
function fieldName(industry, sex) {
  return `${industry} (${sex})`;
}
