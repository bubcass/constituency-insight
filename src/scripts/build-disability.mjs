...
const outputPath = path.join(dataDir, "disability-2022.csv");
...
if (!sexDimension || !geographyDimension) {
...
const outputRows = electoralDivisions.map((geography) => {
  ...
  if (men + women !== combined) {
    throw new Error(`${geography.label}: sex totals do not match`);
  }
  ...
  return {
    ...
    "Persons with a disability": combined,
  };
});

const edTotal = outputRows.reduce(
  (sum, row) => sum + row["Persons with a disability"],
  0,
);
...
console.log(`Validated combined-sex disability counts against CSO table ${TABLE}`);
