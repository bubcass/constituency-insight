...
const outputPath = path.join(dataDir, "education-qualification-2022.csv");
...
const totalQualification = requiredCategory(qualificationByLabel, "Total");
const qualificationLevels = qualifications.filter((d) => d.code !== totalQualification.code);
...
const outputRows = electoralDivisions.map((geography) => {
  ...
  for (const qualification of qualifications) {
    const positions = {
      [qualificationDimension.id]: qualification.position,
    ...
    if (men + women !== combined) {
      throw new Error(`${geography.label}: ${qualification.label} sex totals do not match`);
    }
    row[qualification.label] = combined;
  }

  const levelSum = qualificationLevels.reduce(
    (sum, qualification) => sum + row[qualification.label],
    0,
  );
  if (levelSum !== row[totalQualification.label]) {
    throw new Error(
      `${geography.label}: qualification levels sum to ${levelSum}, expected ${row[totalQualification.label]}`,
    );
  }
...
for (const qualification of qualifications) {
  const edTotal = outputRows.reduce((sum, row) => sum + row[qualification.label], 0);
  ...
  if (edTotal !== nationalTotal) {
    throw new Error(
      `${qualification.label}: ED totals sum to ${edTotal}, but the CSO Ireland total is ${nationalTotal}`,
    );
  }
}
...
console.log(`Validated combined-sex qualification counts against CSO table ${TABLE}`);
